...
from projects.DeepLab.deeplab.axialnet import AxialBlock
...
def build_resnet_deeplab_backbone(cfg, input_shape):
    ...
    if cfg.MODEL.RESNETS.STEM_TYPE == "basic":
        stem = BasicStem(   # 7*7 stem in resnet
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)): #(0,2)(1,3)(2,4)(3,5)
        if stage_idx == 4:
...
def build_axial_deeplab_backbone(cfg, input_shape):
    ...
    # Avoid creating variables without gradients
    # It consumes extra memory and may cause allreduce to fail
    out_stage_idx = [{"layer1": 2, "layer2": 3, "layer3": 4, "layer4": 5}[f] for f in out_features]  #2,3,5
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if stage_idx == 5:
            stage_kargs.pop("dilation")
            stage_kargs["dilation_per_block"] = [dilation * mg for mg in res5_multi_grid] #mg=[1,2,4]   eg dilation5=4, dilationperblock=[4,8,16]
        blocks = ResNet.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
...
if __name__=='__main__':
    ...
    cfg.merge_from_file('/home/wangzhilin/detectron2/projects/DeepLab/configs/Cityscapes-SemanticSegmentation/deeplab_v3_plus_R_103_os16_mg124_poly_90k_bs16.yaml')
    model=build_resnet_deeplab_backbone(cfg)   #resnet backbone requests (N,C,H,W)
    input_shape can not be a simple tuple'''
